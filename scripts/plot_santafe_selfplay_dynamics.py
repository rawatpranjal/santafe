#!/usr/bin/env python3
"""
Generate Santa Fe Self-Play Dynamics Figure for Section 6.

Creates a 4x4 grid showing trading patterns from curated logs for all 13 Santa Fe traders.

Usage:
    python scripts/plot_santafe_selfplay_dynamics.py
    python scripts/plot_santafe_selfplay_dynamics.py --round 1
"""
...
def generate_figure(panels: list[PanelData], output_path: Path, target_round: int) -> None:
    """Generate the 4x4 selfplay dynamics figure (13 traders + 3 empty)."""
    ...
    plt.suptitle(
        f"Santa Fe Self-Play Dynamics (Round {target_round}, Period 1)",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Santa Fe Selfplay Dynamics Figure")
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("paper/arxiv/figures/santafe_selfplay_dynamics.png"),
        help="Output path for figure",
    )

    args = parser.parse_args()

    print(f"Generating Santa Fe selfplay dynamics figure for round {args.round}...")

    panels = []
    for strategy_name, strategy_short in STRATEGIES:
        log_file = LOGS_DIR / f"p2_self_{strategy_short}_base_events.jsonl"
