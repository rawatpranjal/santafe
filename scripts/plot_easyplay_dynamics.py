#!/usr/bin/env python3
"""
Generate Easy-Play Dynamics Figure for Section 5.

Creates a 1x4 grid showing ZI, ZIC, ZI2, ZIP buyers vs TruthTeller sellers with:
- Price scatter (all bids/asks + trade markers)
- Best bid/ask spread overlay
- Highlights inefficiency of ZI vs efficiency of constrained strategies

Usage:
    python scripts/plot_easyplay_dynamics.py
    python scripts/plot_easyplay_dynamics.py --seed 42
"""
...
@dataclass
class PanelData:
    """Data for a single panel in the 1x4 grid."""
...
def run_easyplay_period(strategy: str, seed: int, num_steps: int = 100) -> PanelData:
    """
    Run a single period of easy-play: given strategy buyers vs TruthTeller sellers.

    Args:
        strategy: The buyer strategy type (ZI, ZIC, ZI2, ZIP)
        seed: Random seed
    ...
    # Buyers use the specified strategy
    ...
    # Sellers are TruthTellers (ask at true cost)
    ...
    # Extract all quotes (bids and asks)
    ...
    # Extract trades with profit info
    # raw_trades format: list of (buyer_id, seller_id, price, buyer_unit_index) tuples
    raw_trades = extract_trades_from_orderbook(market.orderbook, num_steps)

    # Get trade times from orderbook
    trade_times = []
    for t in range(1, num_steps + 1):
    ...
    for i, trade in enumerate(raw_trades):
        buyer_id, seller_id, price, buyer_unit_idx = trade
        time = trade_times[i] if i < len(trade_times) else i + 1

        # Find buyer's value for this token
        if buyer_unit_idx < len(buyer_valuations.get(buyer_id, [])):
            buyer_value = buyer_valuations[buyer_id][buyer_unit_idx]
            buyer_profit = buyer_value - price
        else:
            buyer_profit = 0
        panel.trades.append(TradeRecord(time=time, price=price, buyer_profit=buyer_profit))
        if buyer_profit < 0:
            panel.num_losing_trades += 1
    ...
    # Auto-zoom x-axis (show first 20 steps for cleaner visualization)
    if panel.trades:
        last_trade_time = max(t.time for t in panel.trades)
        panel.x_max = min(max(last_trade_time + 5, 15), num_steps)
    else:
        panel.x_max = 20
...
def generate_figure(panels: list[PanelData], output_path: Path, seed: int) -> None:
    """Generate the 1x5 easy-play dynamics figure."""
    ...
    for idx, (panel, ax) in enumerate(zip(panels, axes)):
        ...
        if panel.best_bids:
            ...
            ax.step(bid_t, bid_p, where="post", color="#0066FF", linewidth=2, label="Best Bid")
        if panel.best_asks:
            ask_t, ask_p = zip(*panel.best_asks)
            ax.step(ask_t, ask_p, where="post", color="#FF0000", linewidth=2, label="Best Ask")

        # Plot trades - color by profit (green = profit, red = loss)
        for trade in panel.trades:
            trade_color = "#388E3C" if trade.buyer_profit >= 0 else "#D32F2F"
            ax.scatter(
                [trade.time],
                [trade.price],
                s=120,
        ...
        # CE line (seller's cost region - where TruthTellers ask)
        ax.axhline(
            panel.ce_price,
            color="#388E3C",
            linestyle="--",
        ...
        # Add efficiency and trade info
        info_text = f"Eff: {panel.efficiency:.0f}%\nTrades: {len(panel.trades)}"
        if panel.num_losing_trades > 0:
            info_text += f"\nLosses: {panel.num_losing_trades}"
    ...
    plt.suptitle(
        "Easy-Play: Buyers vs TruthTeller Sellers",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Easy-Play Dynamics Figure")
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("paper/arxiv/figures/easyplay_dynamics.png"),
        help="Output path for figure",
    )

    args = parser.parse_args()

    print(f"Generating easy-play dynamics figure with seed {args.seed}...")

    panels = []
    for strategy in STRATEGIES:
        print(f"  Running {strategy} vs TruthTeller...")
        panel = run_easyplay_period(strategy, args.seed)
        panels.append(panel)
        print(
            f"    Efficiency: {panel.efficiency:.1f}%, Trades: {len(panel.trades)}, Losses: {panel.num_losing_trades}"
        )
