"""
Zero-Intelligence Plus 1 (ZIP1) trading agent.

Based on:
Cliff, D., & Bruten, J. (1997). "Minimal-Intelligence Agents for Bargaining
Behaviors in Market-Based Environments"

The ZIP1 agent uses simple machine learning (Widrow-Hoff delta rule) to adapt
...
class ZIP1(Agent):
    """
    Zero-Intelligence Plus 1 (ZIP1) trader with adaptive profit margins.
    ...
    def _calculate_quote(self) -> int:
        ...
        logger.debug(
            f"ZIP P{self.player_id} QUOTE: limit={limit_price} margin={self.margin:.4f} quote={quote}"
        )
        return quote
    ...
    def _should_lower_margin(
        self, my_price: int, last_price: int, high_bidder: int = 0, low_asker: int = 0
    ) -> bool:
        """
    ...
    def buy_sell_result(
        ...
        if my_quote > 0:  # Have a valid quote
            ...
            if self._should_raise_margin(
                my_quote, trade_price if trade_type != 0 else self.last_shout_price
            ):
                target = self._calculate_target_price(
                    trade_price if trade_type != 0 else self.last_shout_price, raise_margin=True
                )
                self._update_margin(target)

            # Check if should lower margin (only if active)
            elif self._should_lower_margin(
                my_quote,
                trade_price if trade_type != 0 else self.last_shout_price,
                high_bidder,
                low_asker,
            ):
                target = self._calculate_target_price(
                    trade_price if trade_type != 0 else self.last_shout_price, raise_margin=False
                )
                self._update_margin(target)


# ZIP is a proper subclass for correct __name__ attribute
class ZIP(ZIP1):
    """ZIP alias with proper class name for tournament tracking."""
