...
class Tournament:
    ...
    def run(self) -> pd.DataFrame:
        ...
        if token_mode == "uniform":
            ...
            self.logger.info("Using Gode & Sunder style overlapping supply/demand")
        else:
            ...
            for p in range(1, self.config.market.num_periods + 1):
                ...
                while market.current_time < market.num_times:
                    ...
                    if market.orderbook.should_terminate_early():
                        break
                    # Check for global exhaustion (all traders out of tokens)
                    if market.all_traders_exhausted():
                        break
                ...
                seller_costs_list = [
                    seller_costs_dict[a.player_id - len(buyer_types)]
                    for a in agents
                    if not a.is_buyer
                ]
                ...
                if (r == 1 or r == 3) and p == 1 and len(trades) > 0:
                    ...
                    self.logger.info(
                        f"Trades: {len(trades)}, Actual Surplus: {actual_surplus}, Max Surplus: {max_surplus}"
                    )
                    self.logger.info(
                        f"buyer_valuations_dict keys: {list(buyer_valuations_dict.keys())}"
                    )
                    self.logger.info(f"seller_costs_dict keys: {list(seller_costs_dict.keys())}")

                    # Print ALL valuations for Round 3
                    if r == 3:
                        self.logger.info("ALL BUYER VALUATIONS:")
                        for bid, vals in buyer_valuations_dict.items():
                            self.logger.info(f"  Buyer {bid}: {vals}")
                        self.logger.info("ALL SELLER COSTS:")
                        ...
                        self.logger.info("AGENT VALUATIONS (from agent objects):")
                        for agent in agents:
                            if agent.is_buyer:
                                self.logger.info(
                                    f"  Agent {agent.player_id} (buyer): {agent.valuations}"
                                )
                            else:
                                self.logger.info(
                                    f"  Agent {agent.player_id} (seller): {agent.valuations}"
                                )
                    ...
                    self.logger.info(
                        f"Agent profits: buyers={total_buyer_profit}, sellers={total_seller_profit}, total={total_buyer_profit+total_seller_profit}"
                    )
                    ...
                    if r == 3:
                        ...
                        for trade_idx, (bid, sid, price, bunit) in enumerate(trades):
                            ...
                            if trade_idx < 10:
                                self.logger.info(
                                    f"    Manual Trade {trade_idx}: B{bid}[{bunit}]={bval}, S{sid}[{sunit}]={scost}, price={price}, surplus={trade_surplus}"
                                )
                        ...
                        self.logger.info(
                            f"  Agent profit total: {total_buyer_profit + total_seller_profit}"
                        )
                    self.logger.info("=== END DIAGNOSTIC ===\n")

                # Calculate Decomposition Metrics
                v_inefficiency = calculate_v_inefficiency(
                    actual_trades, buyer_vals_list, seller_costs_list
                )
                em_inefficiency = calculate_em_inefficiency(
                    trades, buyer_valuations_dict, seller_costs_dict
                )
                ...
                equilibrium_price = (
                    int(sum(transaction_prices) / len(transaction_prices))
                ...
                price_mean = (
                    float(sum(transaction_prices) / len(transaction_prices))
                    if transaction_prices
                    else 0.0
                )
