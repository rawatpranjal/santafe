...
class PPOTrader(BaseTrader): # Consider renaming to PPO_LSTM_Trader if preferred
    ...
    def _map_action_to_price(self, action_idx):
        """ Maps action index (0 to action_dim-1) to a price or None ('pass'). """
        if action_idx == self.action_dim - 1: # 'pass' action is the last index
            return None

        val_cost = self.get_next_value_cost()
        if val_cost is None:
            # self.logger.warning("Cannot map action to price, no value/cost available.")
            return None # Can't quote without a reference

        num_price_levels = self.action_dim - 1
        if num_price_levels <= 0: return None # Should not happen

        # Map action_idx (0 to num_price_levels-1) to a relative position [-0.5, 0.5]
        if num_price_levels == 1:
             relative_pos = 0.0 # Center the single price action
        else:
             norm_pos = action_idx / (num_price_levels - 1) # Range [0, 1]
             relative_pos = norm_pos - 0.5 # Range [-0.5, 0.5]

        # Calculate price span based on market range and configured percentage
        price_span = (self.max_price - self.min_price) * self.price_range_pct

        # Calculate delta based on role and relative position
        # Multiplier maps [-0.5, 0.5] to [0, 1]
        multiplier = relative_pos + 0.5
        if self.is_buyer:
            price_delta = -price_span * multiplier # More negative multiplier -> smaller delta -> higher bid
        else: # Seller
            price_delta = price_span * multiplier # More positive multiplier -> larger delta -> higher ask

        target_price = val_cost + price_delta

        # Ensure price is within profitable bounds and market limits
        if self.is_buyer:
             # Bid must be <= value and >= min_price
             final_price = max(self.min_price, min(int(round(target_price)), val_cost))
        else: # Seller
             # Ask must be >= cost and <= max_price
             final_price = min(self.max_price, max(int(round(target_price)), val_cost))

        # Final clamp to absolute market bounds (safety check)
