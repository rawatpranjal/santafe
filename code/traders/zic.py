...
class ZICBuyer(BaseTrader):
    ...
    def __init__(self, name, is_buyer, private_values, **kwargs):
        ...
        self.shout_probability = kwargs.get('shout_probability', 0.8) # Probability of submitting a quote
    ...
    def make_bid_or_ask(self, current_bid_info, current_ask_info, phibid, phiask, market_history):
        ...
        if value is None: self.logger.warning("Can trade but get_next_value_cost returned None."); return None

        min_possible_bid = self.min_price; max_possible_bid = value
        if max_possible_bid < min_possible_bid: self.logger.debug(f"Cannot bid profitably (V {value} < MinP {min_possible_bid})"); return None
        ...
        if random.random() < self.shout_probability:
            try:
                if min_possible_bid <= max_possible_bid:
                    ...
                    return bid_price
                else: self.logger.warning(f"Invalid range for randint bid [{min_possible_bid},{max_possible_bid}]"); return None
            except ValueError: self.logger.warning(f"Value error generating bid for range [{min_possible_bid},{max_possible_bid}]"); return None
        else: self.logger.debug("Decided not to shout bid."); return None

    def request_buy(self, current_offer_price, current_bid_info, current_ask_info, phibid, phiask, market_history):
        """Accept any offer <= own value."""
...
class ZICSeller(BaseTrader):
    ...
    def make_bid_or_ask(self, current_bid_info, current_ask_info, phibid, phiask, market_history):
        ...
        if cost is None: self.logger.warning("Can trade but get_next_value_cost returned None."); return None

        min_possible_ask = cost; max_possible_ask = self.max_price
        if min_possible_ask > max_possible_ask: self.logger.debug(f"Cannot ask profitably (C {cost} > MaxP {max_possible_ask})"); return None
        ...
        if random.random() < self.shout_probability:
            try:
                if min_possible_ask <= max_possible_ask:
                    ...
                    return ask_price
                else: self.logger.warning(f"Invalid range for randint ask [{min_possible_ask},{max_possible_ask}]"); return None
            except ValueError: self.logger.warning(f"Value error generating ask for range [{min_possible_ask},{max_possible_ask}]"); return None
        else: self.logger.debug("Decided not to shout ask."); return None
