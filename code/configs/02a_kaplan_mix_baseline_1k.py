# 02a_kaplan_mix_baseline_1k.py
# Kaplan performance replication - Baseline Mix (5B/5S, 4T, 25S)
# Uses Round=Episode structure

import random
import numpy as np

# --- Timing and Phases ---
TOTAL_ROUNDS = 100 # Number of rounds (episodes)
TRAINING_ROUNDS = 0 # No RL training for these baseline agents
EVALUATION_ROUNDS = TOTAL_ROUNDS

STEPS_PER_PERIOD = 25 # Standard SFI steps per period
NUM_PERIODS = 3     # Standard SFI periods per round

# --- CONFIG Dictionary ---
CONFIG = {
    # --- Experiment Identification ---
    "experiment_name": "02a_kaplan_mix_baseline_1k", # Will be overridden by runner
    "experiment_dir": "experiments",

    # --- Auction Settings ---
    "num_rounds": TOTAL_ROUNDS,
    "num_periods": NUM_PERIODS,
    "num_steps": STEPS_PER_PERIOD,
    "num_training_rounds": TRAINING_ROUNDS,
    "num_buyers": 5,    # 5 Buyers
    "num_sellers": 5,   # 5 Sellers
    "num_tokens": 4,    # Standard tokens per period
    "min_price": 1,
    "max_price": 2000,
    "gametype": 6453,   # Standard SFI gametype

    # --- Trader Configuration ---
    # Mix: 1 KP, 1 ZIP, 1 GD, 1 EL, 1 ZIC per side
    "buyers": [
        {"type": "kaplan"},
        {"type": "zip"},
        {"type": "gd"},  # Using simplified GD
        {"type": "el"},  # Using EL with inter-period update
        {"type": "zic"},
    ],
    "sellers": [
        {"type": "kaplan"},
        {"type": "zip"},
        {"type": "gd"},  # Using simplified GD
        {"type": "el"},  # Using EL with inter-period update
        {"type": "zic"},
    ],

    # --- PPO/RL Hyperparameters (Not used) ---
    "rl_params": {},

    # --- RNG Seeds, Logging, Plotting, Saving ---
    "rng_seed_values": 2027, # Consistent seed set for 02 series
    "rng_seed_auction": 414,
    "rng_seed_rl": 525,
    "log_level": "INFO",       # Console log level
    "log_level_rl": "WARNING", # RL logic logs (e.g., PPO updates)
    "log_to_file": True,
    "generate_per_round_plots": False, # Typically off for long runs
    "generate_eval_behavior_plots": False, # No RL agents to plot
    "num_eval_rounds_to_plot": 0,
    "save_rl_model": False,
    "load_rl_model_path": None,
}