...
try:
    ...
    from src.traders.registry import get_trader_class, available_strategies
...
def main():
    ...
    try:
        # Open CSV file for streaming step logs
        step_log_path = os.path.join(exp_path, "step_log_all.csv")
        step_log_file = open(step_log_path, 'w', newline='')
        logger.info(f"Opened step log file for streaming: {step_log_path}")
        
        # Pass the modified config dict containing class objects and step log file
        auction_instance = Auction(CONFIG, step_log_file=step_log_file)
    ...
    # Close step log file if it was opened
    if step_log_file:
        try:
            step_log_file.close()
            logger.info("Closed step log file.")
        except Exception as e:
            logger.error(f"Error closing step log file: {e}")
    ...
    # Step logs are now in CSV file, not in memory
    all_step_logs = []  # Empty list since we're streaming to CSV
    ...
    # Step logs are already saved to CSV during execution
    if os.path.exists(os.path.join(exp_path, "step_log_all.csv")):
        logger.info(f"Step logs already saved to step_log_all.csv during execution")
        # Read it back if needed for analysis
        try:
            dfLogs_all = pd.read_csv(os.path.join(exp_path, "step_log_all.csv"))
            logger.info(f"Loaded {len(dfLogs_all)} step logs from CSV for analysis")
        except Exception as e:
            logger.warning(f"Could not load step logs for analysis: {e}")
            dfLogs_all = None
    else:
        dfLogs_all = None
    
    # Skip the old step log saving code
    if False and all_step_logs:  # This block is now disabled
        try:
